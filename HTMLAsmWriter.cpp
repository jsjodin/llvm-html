//===- HTMLAsmWriter.cpp - Printing LLVM as HTML file--- ------------------===//
//
...
static void PrintLLVMName(raw_ostream &OS, StringRef Name, PrefixType Prefix) {
  ...
  if (Prefix == NoPrefix || Prefix == LabelPrefix)
...
class HTMLAssemblyWriter {
  ...
  std::set<uint64_t> KnownHTMLTags;
  std::map<uint64_t, std::set<uint64_t> > DefToUseMap;

public:
  /// Construct an HTMLAssemblyWriter with an external SlotTracker
  HTMLAssemblyWriter(formatted_raw_ostream &o, formatted_raw_ostream &cssos,
                     std::string cssfilename, SlotTracker &Mac, const Module *M,
                     AssemblyAnnotationWriter *AAW, bool IsForDebug,
                     bool ShouldPreserveUseListOrder = false);

  HTMLAssemblyWriter(formatted_raw_ostream &o, formatted_raw_ostream &csso,
                     std::string cssfilename, SlotTracker &Mac,
  ...
  std::string getHTMLLinkId(uint64_t Tag);
  std::string getHTMLId(uint64_t tag);
  uint64_t getHTMLTag(const void *P);
  void printHTMLMainStyles();
  void printHTMLTagsStyles();
  void printHTMLStart(const std::string Title);
  void printCSSDefLinks();

  void printHTMLEnd();
  void printHTMLLink(std::string Text, std::string URL);
  void printHTMLTagLink(const std::string Text, const uint64_t Tag);
  void printHTMLTag(std::string Text, const void* Tag);
  void printHTMLTag(std::string Text, uint64_t Tag);
  void printHTMLOperand(std::string Text, const Value *V);
  void printHTMLOperand(std::string Text, uint64_t Tag);
  void printHTMLLLVMName(raw_ostream &OS, const Value *V, bool IsDef = false);
  void printHTMLLLVMName(raw_ostream &OS, StringRef Name, PrefixType Prefix, uint64_t Tag, bool IsDef = false);
  void collectAllHTMLFunctionTags(const Module *M);
  void collectAllHTMLBodyTags(const Function *F);
  std::string getModuleDocURL() {
    return "https://llvm.org/docs/LangRef.html#module-structure";
  }
  std::string getFunctionDocURL() {
    return "https://llvm.org/docs/LangRef.html#functions";
  }

  std::string getGlobalValueDocUrl() {
    return "https://llvm.org/docs/LangRef.html#linkage-types";
  }

  std::string getVisibilityTypesDocURL() {
    return "https://llvm.org/docs/LangRef.html#visibility-styles";
  }
  ...
  void WriteAsOperandInternal2(raw_ostream &Out, const Value *V,
                               AsmWriterContext &WriterCtx, bool IsDef = false);
  void WriteAsOperandInternal2(raw_ostream &Out, const Metadata *MD,
                              AsmWriterContext &WriterCtx,
                              bool FromValue = false, bool IsDef = false);
...
// TypePrinting, etc.
void HTMLAssemblyWriter::WriteAsOperandInternal2(raw_ostream &Out,
                                                 const Value *V,
                                                 AsmWriterContext &WriterCtx,
                                                 bool IsDef) {
  if (V->hasName()) {
    std::string NameString;
    raw_string_ostream NameOS(NameString);
    PrintLLVMName(NameOS, V);
    if (IsDef)
      printHTMLTag(NameOS.str(), getHTMLTag(V));
    else
      printHTMLOperand(NameOS.str(), V);
    return;
  }
  ...
  PrefixType PrefixT = LocalPrefix;
  int Slot;
  ...
  if (Slot != -1)
    printHTMLLLVMName(Out, std::to_string(Slot), PrefixT, getHTMLTag(V), IsDef);
  else
    Out << "<badref>";
}
...
HTMLAssemblyWriter::HTMLAssemblyWriter(formatted_raw_ostream &o,
                                       formatted_raw_ostream &csso,
                                       std::string cssfilename,
                                       SlotTracker &Mac, const Module *M,
                                       AssemblyAnnotationWriter *AAW,
                                       bool IsForDebug,
                                       bool ShouldPreserveUseListOrder)
    : Out(o), CSSOut(csso), CSSFileName(cssfilename), TheModule(M), Machine(Mac), TypePrinter(M), AnnotationWriter(AAW),
...
HTMLAssemblyWriter::HTMLAssemblyWriter(formatted_raw_ostream &o,
                                       ...
                                       SlotTracker &Mac,
                               const ModuleSummaryIndex *Index, bool IsForDebug)
  : Out(o), CSSOut(csso), CSSFileName(cssfilename), TheIndex(Index), Machine(Mac), TypePrinter(/*Module=*/nullptr),
      IsForDebug(IsForDebug), ShouldPreserveUseListOrder(false) {}

void HTMLAssemblyWriter::writeOperand(const Value *Operand, bool PrintType) {
  if (!Operand) {
    Out << "<null operand!>";
    return;
  }
...
void HTMLAssemblyWriter::writeSyncScope(const LLVMContext &Context,
                                    SyncScope::ID SSID) {
  switch (SSID) {
...
void HTMLAssemblyWriter::writeAtomic(const LLVMContext &Context,
                                 AtomicOrdering Ordering,
                                 SyncScope::ID SSID) {
...
void HTMLAssemblyWriter::writeAtomicCmpXchg(const LLVMContext &Context,
                                        AtomicOrdering SuccessOrdering,
                                        AtomicOrdering FailureOrdering,
                                        SyncScope::ID SSID) {
...
void HTMLAssemblyWriter::writeParamOperand(const Value *Operand,
                                       AttributeSet Attrs) {
  if (!Operand) {
    Out << "<null operand!>";
    return;
...
void HTMLAssemblyWriter::printHTMLMainStyles() {
  CSSOut << "a:link {\n";
  CSSOut << "  color:black;\n";
  CSSOut << "  text-decoration:none;\n";
  CSSOut << "}\n";
  CSSOut << "a:visited {\n";
  CSSOut << "  color:black;\n";
  CSSOut << "  text-decoration:none;\n";
...
void HTMLAssemblyWriter::printHTMLTagsStyles() {
  for (auto Tag : KnownHTMLTags) {
    CSSOut << "#" << getHTMLId(Tag) << " {";
    CSSOut << " background-color: #fff; }\n";
    CSSOut << "#" << getHTMLId(Tag) << ":target {";
    CSSOut << " background-color: #ffa; }\n";
  }
}

void HTMLAssemblyWriter::printHTMLStart(const std::string Title) {
  Out << "<!DOCTYPE html>\n";
  Out << "<html>\n";
  Out << "<head>\n";
  printHTMLMainStyles();
  ...
  Out << "<body>\n";
  Out << "<h1>";
  printHTMLLink(Title, getModuleDocURL());
  Out << "</h1>\n";
  Out << "<pre>\n";
}

void HTMLAssemblyWriter::printCSSDefLinks() {
  for (auto &DefUse : DefToUseMap) {
    ...
    for (auto &LTag : DefUse.second) {
      if (IsFirst)
        ...
        CSSOut << ", ";
      CSSOut << "#" << getHTMLId(Tag) << ":hover ~ #" << getHTMLLinkId(LTag);
    }
    CSSOut << " { background-color: #ffa;}\n";
  }
}
...
std::string HTMLAssemblyWriter::getHTMLId(uint64_t Tag) {
  std::string Id("tag");
  Id += std::to_string(Tag);
  return Id;
}

std::string HTMLAssemblyWriter::getHTMLLinkId(uint64_t Tag) {
  std::string Id("ltag");
  Id += std::to_string(Tag);
  return Id;
}

uint64_t HTMLAssemblyWriter::getHTMLTag(const void *P) {
  return (uint64_t)P;
}

void HTMLAssemblyWriter::printHTMLLink(const std::string Text,
                                       const std::string URL) {
  static uint64_t count = 0;
  //  Out << "<a href=\"" << URL << "\" style=\"text-decoration:none\" target=\"_blank\">" << Text << "</a>";
  //  Out << "<a href=\"" << URL << "\" >" << Text << "</a>";
  if(URL.rfind("#", 0) == 0) {
    Out << "<a id=\"" << getHTMLLinkId(count) << "\" href=\"" << URL << "\" >" << Text << "</a>";
    CSSOut << "#" << getHTMLLinkId(count) << ":hover ~ " << URL << " {";
    CSSOut << " background-color: #ffa; }\n";
    CSSOut << URL << ":has(~ #" << getHTMLLinkId(count) << ":hover) {";
    CSSOut << " background-color: #ffa; }\n";
    count ++;
  } else {
    Out << "<a href=\"" << URL << "\" style=\"text-decoration:none\" >" << Text << "</a>";
  }
}

void HTMLAssemblyWriter::printHTMLTagLink(const std::string Text,
                                          const uint64_t Tag) {
  std::string URL;
  URL+="#";
  URL+=getHTMLId(Tag);
  ...
  //  Out << "<a href=\"" << URL << "\" >" << Text << "</a>";
  if(URL.rfind("#", 0) == 0) {
    Out << "<a id=\"" << getHTMLLinkId(count) << "\" href=\"" << URL << "\" style=\"text-decoration:none\" >" << Text << "</a>";
    CSSOut << "#" << getHTMLLinkId(count) << ":hover ~ " << URL << " {";
    CSSOut << " background-color: #ffa; }\n";
    CSSOut << URL << ":has(~ #" << getHTMLLinkId(count) << ":hover) {";
    CSSOut << " background-color: #ffa; }\n";
    DefToUseMap[Tag].insert(count);
    count ++;
  } else {
    Out << "<a href=\"" << URL << "\" style=\"text-decoration:none\" >" << Text << "</a>";
  }
...
void HTMLAssemblyWriter::printHTMLTag(std::string Text, const void* Tag) {
  printHTMLTag(Text, (uint64_t)(Tag));
}
...
void HTMLAssemblyWriter::printHTMLOperand(std::string Text, const Value *V) {
  uint64_t Tag = getHTMLTag(V);
  printHTMLOperand(Text, Tag);
}
...
void HTMLAssemblyWriter::printHTMLLLVMName(raw_ostream &OS, const Value* V, bool IsDef) {
  uint64_t Tag = getHTMLTag(V);
  if (KnownHTMLTags.count(Tag) == 0) {
    PrintLLVMName(OS, V);
    return;
  ...
  PrintLLVMName(NameOS, V);
  if (IsDef)
    printHTMLTag(NameOS.str(), Tag);
  else
    printHTMLOperand(NameOS.str(), Tag);
}

void HTMLAssemblyWriter::printHTMLLLVMName(raw_ostream &OS, StringRef Name, PrefixType Prefix, uint64_t Tag, bool IsDef) {
  if (KnownHTMLTags.count(Tag) == 0) {
    PrintLLVMName(OS, Name, Prefix);
    return;
  }

  std::string NameString;
  raw_string_ostream NameOS(NameString);
  PrintLLVMName(NameOS, Name, Prefix);
  if (IsDef)
    printHTMLTag(NameOS.str(), Tag);
  else
    printHTMLOperand(NameOS.str(), Tag);
...
void HTMLAssemblyWriter::collectAllHTMLFunctionTags(const Module *M) {
  for (const Function &F : *M) {
    uint64_t Tag = getHTMLTag(&F);
    KnownHTMLTags.insert(Tag);
...
void HTMLAssemblyWriter::collectAllHTMLBodyTags(const Function *F) {
  ...
  for (const BasicBlock &BB : *F) {
    ...
    for (const Instruction &I : BB) {
      auto InstTag = getHTMLTag(&I);
      KnownHTMLTags.insert(InstTag);
    }
  }
}

void HTMLAssemblyWriter::printModule(const Module *M) {
  collectAllHTMLFunctionTags(M);
  ...
  if (!M->getModuleIdentifier().empty() &&
    ...
    printHTMLStart(M->getModuleIdentifier());
    Out << "; ModuleID = '" << M->getModuleIdentifier() << "'\n";
  } else {
    printHTMLStart("Module");
  }
...
void HTMLAssemblyWriter::printModuleSummaryIndex() {
  assert(TheIndex);
...
void HTMLAssemblyWriter::printTypeTestResolution(const TypeTestResolution &TTRes) {
...
void HTMLAssemblyWriter::printTypeIdSummary(const TypeIdSummary &TIS) {
  Out << ", summary: (";
...
void HTMLAssemblyWriter::printTypeIdCompatibleVtableSummary(
...
void HTMLAssemblyWriter::printArgs(const std::vector<uint64_t> &Args) {
  Out << "args: (";
  FieldSeparator FS;
  for (auto arg : Args) {
    Out << FS;
...
void HTMLAssemblyWriter::printWPDRes(const WholeProgramDevirtResolution &WPDRes) {
  Out << "wpdRes: (kind: ";
...
void HTMLAssemblyWriter::printAliasSummary(const AliasSummary *AS) {
  Out << ", aliasee: ";
...
void HTMLAssemblyWriter::printGlobalVarSummary(const GlobalVarSummary *GS) {
...
void HTMLAssemblyWriter::printFunctionSummary(const FunctionSummary *FS) {
...
void HTMLAssemblyWriter::printTypeIdInfo(
...
void HTMLAssemblyWriter::printVFuncId(const FunctionSummary::VFuncId VFId) {
...
void HTMLAssemblyWriter::printNonConstVCalls(
...
void HTMLAssemblyWriter::printConstVCalls(
...
void HTMLAssemblyWriter::printSummary(const GlobalValueSummary &Summary) {
...
void HTMLAssemblyWriter::printSummaryInfo(unsigned Slot, const ValueInfo &VI) {
  Out << "^" << Slot << " = gv: (";
  if (!VI.name().empty())
    Out << "name: \"" << VI.name() << "\"";
  else
...
void HTMLAssemblyWriter::printNamedMDNode(const NamedMDNode *NMD) {
  Out << '!';
...
void HTMLAssemblyWriter::printGlobal(const GlobalVariable *GV) {
  if (GV->isMaterializable())
    Out << "; Materializable\n";

  AsmWriterContext WriterCtx(&TypePrinter, &Machine, GV->getParent());
  WriteAsOperandInternal2(Out, GV, WriterCtx, true);
  Out << " = ";
...
void HTMLAssemblyWriter::printAlias(const GlobalAlias *GA) {
...
void HTMLAssemblyWriter::printIFunc(const GlobalIFunc *GI) {
...
void HTMLAssemblyWriter::printComdat(const Comdat *C) {
  C->print(Out);
}

void HTMLAssemblyWriter::printTypeIdentities() {
...
void HTMLAssemblyWriter::printFunction(const Function *F) {
  ...
  std::string NameString;
  raw_string_ostream NameOS(NameString);
  WriteAsOperandInternal(NameOS, F, WriterCtx);
  printHTMLTag(NameOS.str(), F);
  Out << '(';
...
void HTMLAssemblyWriter::printArgument(const Argument *Arg, AttributeSet Attrs) {
  // Output type...
...
void HTMLAssemblyWriter::printBasicBlock(const BasicBlock *BB) {
  bool IsEntryBlock = BB->getParent() && BB->isEntryBlock();
  if (BB->hasName()) {              // Print out the label if it exists...
    Out << "\n";
    printHTMLLLVMName(Out, BB->getName(), LabelPrefix, getHTMLTag(BB), true);
    Out << ':';
...
void HTMLAssemblyWriter::printInstructionLine(const Instruction &I) {
  printInstruction(I);
  Out << '\n';
}
...
/// printGCRelocateComment - print comment after call to the gc.relocate
/// intrinsic indicating base and derived pointer names.
void HTMLAssemblyWriter::printGCRelocateComment(const GCRelocateInst &Relocate) {
  Out << " ; (";
...
/// printInfoComment - Print a little comment after the instruction indicating
/// which slot it occupies.
void HTMLAssemblyWriter::printInfoComment(const Value &V) {
...
void HTMLAssemblyWriter::printInstruction(const Instruction &I) {
  ...
  if (I.hasName()) {
    ...
    if (SlotNum == -1)
      ...
      printHTMLLLVMName(Out, std::to_string(SlotNum), LocalPrefix, getHTMLTag(&I), true);
      Out << " = ";
    }
  }
...
void HTMLAssemblyWriter::printMetadataAttachments(
    const SmallVectorImpl<std::pair<unsigned, MDNode *>> &MDs,
    StringRef Separator) {
  if (MDs.empty())
    return;
...
void HTMLAssemblyWriter::writeMDNode(unsigned Slot, const MDNode *Node) {
  Out << '!' << Slot << " = ";
  printMDNodeBody(Node);
  Out << "\n";
}

void HTMLAssemblyWriter::writeAllMDNodes() {
...
void HTMLAssemblyWriter::printMDNodeBody(const MDNode *Node) {
  auto WriterCtx = getContext();
  WriteMDNodeBodyInternal(Out, Node, WriterCtx);
}

void HTMLAssemblyWriter::writeAttribute(const Attribute &Attr, bool InAttrGroup) {
...
void HTMLAssemblyWriter::writeAttributeSet(const AttributeSet &AttrSet,
                                       bool InAttrGroup) {
  bool FirstAttr = true;
...
void HTMLAssemblyWriter::writeAllAttributeGroups() {
...
void HTMLAssemblyWriter::printUseListOrder(const Value *V,
...
void HTMLAssemblyWriter::printUseLists(const Function *F) {
...
void HTMLWriter::print(raw_ostream &ROS, raw_ostream &RCSSOS,
                       std::string CSSFileName,
                       AssemblyAnnotationWriter *AAW,
                       bool ShouldPreserveUseListOrder, bool IsForDebug) const {
  SlotTracker SlotTable(M);
  formatted_raw_ostream OS(ROS);
  formatted_raw_ostream CSSOS(RCSSOS);
  HTMLAssemblyWriter W(OS, CSSOS, CSSFileName, SlotTable, M, AAW, IsForDebug,
...
void NamedMDNode::print(raw_ostream &ROS, bool IsForDebug) const {
  ...
  formatted_raw_ostream OS(ROS);
  HTMLAssemblyWriter W(OS, SlotTable, getParent(), nullptr, IsForDebug);
...
void Value::print(raw_ostream &ROS, ModuleSlotTracker &MST,
  ...
  if (const Instruction *I = dyn_cast<Instruction>(this)) {
    ...
    HTMLAssemblyWriter W(OS, SlotTable, getModuleFromVal(I), nullptr, IsForDebug);
    W.printInstruction(*I);
  } else if (const BasicBlock *BB = dyn_cast<BasicBlock>(this)) {
    incorporateFunction(BB->getParent());
    HTMLAssemblyWriter W(OS, SlotTable, getModuleFromVal(BB), nullptr, IsForDebug);
    W.printBasicBlock(BB);
  } else if (const GlobalValue *GV = dyn_cast<GlobalValue>(this)) {
    HTMLAssemblyWriter W(OS, SlotTable, GV->getParent(), nullptr, IsForDebug);
...
void ModuleSummaryIndex::print(raw_ostream &ROS, bool IsForDebug) const {
  ...
  formatted_raw_ostream OS(ROS);
  HTMLAssemblyWriter W(OS, SlotTable, this, IsForDebug);
